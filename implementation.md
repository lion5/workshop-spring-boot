...
The `hasErrors()` method called on the `errors` object enables a user to check if some validation errors are present and return to the `/movies` page.
...
Spring Boot eases our lifes a lot since we only have to implement interfaces extending **JpaRepository<MODEL, ID>** for these repository classes. *MODEL* means the class, e.g. `Actor` or `Movie`, and *ID* the datatype of the id attribute.
